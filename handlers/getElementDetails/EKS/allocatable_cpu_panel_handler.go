...
func GetAllocatableCpuPanel(w http.ResponseWriter, r *http.Request) {
	...
	if authFlag {
		...
		jsonString, cloudwatchMetricData, err := EKS.GetAllocatableCPUData(cmd, clientAuth)
		if err != nil {
			http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
			return
		}
		if elementType == "ContainerInsights" && query == "allocatable_cpu_panel" && responseType == "frame" {
