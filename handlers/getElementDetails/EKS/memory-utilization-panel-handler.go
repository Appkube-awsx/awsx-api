...
func GetEKSMemoryUtilizationPanel(w http.ResponseWriter, r *http.Request) {
	...
	if authFlag {
		...
		jsonString, cloudwatchMetricData, err := EKS.GeteksMemoryUtilizationPanel(cmd, clientAuth,nil)
		if err != nil {
			http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
			return
		}
		if elementType == "ContainerInsights" && query == "memory_utilization_panel" {
