...
import (
 ...
 type LatencyData struct{
	TotalRequestLatencyData   float64 `json:"TotalRequestLatencyData"`
	FirstByteLatency  float64 `json:"FirstByteLatency"`
 }
...
func GetLatencyDataPanel(w http.ResponseWriter, r *http.Request) {
	...
	if clientAuth != nil {
		...
		jsonString, cloudwatchMetricData, err := S3.GetLatencyPanel(cmd, clientAuth, cloudwatchClient)
		if err != nil {
		...
		if responseType == "frame" {
			...
			err := json.Unmarshal([]byte(jsonString), &LatencyData)
			if err != nil {
				http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
				return
			}
			...
			jsonBytes, err := json.Marshal(LatencyData)
			if err != nil {
				http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
				return
			}
