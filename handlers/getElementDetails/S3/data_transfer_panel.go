...
import (
 ...
 type datatransferdata struct{
	BytesUploadedData   float64 `json:"BytesUploadedData"`
	BytesDownloadedData  float64 `json:"BytesDownloadedData"`
 }

var (
	authCaches       sync.Map
	clientCaches     sync.Map
	authCacheLocks   sync.RWMutex
	clientCacheLocks sync.RWMutex
)

func GetDatatransferPanel(w http.ResponseWriter, r *http.Request) {
	...
	cloudwatchClient, err := cloudwatchClientCaches(*clientAuth)
	if err != nil {
	...
	if clientAuth != nil {
		...
		jsonString, cloudwatchMetricData, err := S3.GetDataTransferData(cmd, clientAuth, cloudwatchClient)
		if err != nil {
		...
		if responseType == "frame" {
			...
			var datatransferdata []datatransferdata
			err := json.Unmarshal([]byte(jsonString), &datatransferdata)
			if err != nil {
				http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
				return
			}
			...
			jsonBytes, err := json.Marshal(datatransferdata)
			if err != nil {
				http.Error(w, fmt.Sprintf("Exception: %s", err), http.StatusInternalServerError)
				return
			}
...
func authenticateAndCaches(commandParam model.CommandParam) (*model.Auth, error) {
	...
	authCacheLocks.Lock()
	defer authCacheLocks.Unlock()

	if auth, ok := authCaches.Load(cacheKey); ok {
	...
	authCaches.Store(cacheKey, clientAuth)
	return clientAuth, nil
}

func cloudwatchClientCaches(clientAuth model.Auth) (*cloudwatch.CloudWatch, error) {
	cacheKey := clientAuth.CrossAccountRoleArn

	clientCacheLocks.Lock()
	defer clientCacheLocks.Unlock()

	if client, ok := clientCaches.Load(cacheKey); ok {
	...
	clientCaches.Store(cacheKey, cloudWatchClient)
